...
function handleChatMessage(player, messageType, message, receiver)
    ...
    elseif messageType == 1 then -- Action message (/me)
        ...
        -- Format: * Player name action text *
        outputChatBox(string.format("#%.2X%.2X%.2X* %s %s *", r, g, b, playerName, message), root, 255, 255, 255, true)
    ...
    elseif messageType == 4 then -- Internal message (system messages, server notifications)
        -- These are typically generated by the server or scripts
        -- Usually no player source is needed, but we'll include handling just in case
        outputChatBox(message, root, 255, 255, 100, true) -- Yellow text for system messages
